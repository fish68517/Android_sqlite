...
public class BorrowAdapter extends RecyclerView.Adapter<BorrowAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull BorrowAdapter.ViewHolder holder, int position) {
        ...
        if (TextUtils.isEmpty(borrow.getReturnDate())) {
            ...
            holder.tvReturnDate.setTextColor(ContextCompat.getColor(context, R.color.secondary));
            // 添加点击反馈效果
            holder.tvReturnDate.setBackgroundResource(R.drawable.bg_clickable_text);
            holder.tvReturnDate.setPadding(16, 8, 16, 8);
        } else {
            holder.tvReturnDate.setText(borrow.getReturnDate());
            holder.tvReturnDate.setTextColor(ContextCompat.getColor(context, R.color.text_secondary));
            holder.tvReturnDate.setBackground(null);
            holder.tvReturnDate.setPadding(4, 4, 4, 4);
        }
