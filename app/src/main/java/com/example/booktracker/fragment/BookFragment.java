...
import com.example.booktracker.activity.OrderCreateActivity;
    ...
    private void initView() {
        ...
        bookAdapter.setOnItemClickListener(new BookAdapter.OnItemClickListener() {
            ...
            public void onItemBorrowClick(int position, Book book) {
                new MaterialAlertDialogBuilder(getContext(), R.style.CustomMaterialDialog)
                    .setTitle("提示")
                    .setMessage("确定借阅该书籍吗?")
            ...
            @Override
            public void onItemPurchaseClick(int position, Book book) {
                if (book.getRemain() <= 0) {
                    Toast.makeText(getContext(), "图书库存不足", Toast.LENGTH_SHORT).show();
                    return;
                }
                
                // 跳转到订单创建页面
                Intent intent = new Intent(getContext(), OrderCreateActivity.class);
                intent.putExtra("book", book);
                startActivity(intent);
            }
        });
